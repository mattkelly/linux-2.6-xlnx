 ...
 * worst case.
 */
#define XHI_MAX_RETRIES     50000		// was 5000
...
static inline void fifo_icap_start_config(struct hwicap_drvdata *drvdata)
{
	...
	//printk("configuration started 2\n");
}
		...
		u32 *frame_buffer, u32 num_words)
{
	...
	//printk("Entering fifo_set_configuration\n");
	//dev_dbg(drvdata->dev, "fifo_set_configuration\n");
	//printk("Number of words: %d\n", num_words);
  ...
  //printk("Waiting for words to be written\n");
	while (remaining_words > 0) {
		/*
		 * Wait until we have some data in the fifo.
		 */
		//printk("Waiting for vacancy\n");
		...
		while ((write_fifo_vacancy != 0) &&
			...
			write_fifo_vacancy--;
			frame_buffer++;
			//printk("Remaining words: %d\n", remaining_words);
		}
		/* Start pushing whatever is in the FIFO into the ICAP. */
		//printk("Starting config\n");
		fifo_icap_start_config(drvdata);
		//printk("Started config\n");
	}

	//printk("Waiting for write to finish\n");
	...
	//printk("done fifo_set_configuration\n");
